...
| `k_auth_can_i` | none | `verb`, `resource`, `name`, `namespace`, `as_user`, `as_group`, `list_all` | N/A — single-check mode returns `{"allowed": bool}` plus the literal command; `list_all=True` returns `{"permissions": {...}}`. **Still completely broken on every real call** — `KNOWN_ISSUES.md` Issue 40. An earlier `discovery_cache` kwarg mismatch was fixed, but `_dispatch()`'s own `verb` parameter collides with this tool's own `verb` argument, a second, independent bug never touched by that fix. Not yet fixed. | readonly |
...
**Shipped, but still non-functional through the real server — see `KNOWN_ISSUES.md` Issue 40 and Issue 41 (2026-09-25):** the implementation itself (the `run_kubectl`-not-`_checked` exit-code handling this section anticipated as the hard part) is correct and well-tested. `handle_auth_can_i()` originally crashed on every real call because its signature never accepted the `discovery_cache` parameter `server.py`'s `_dispatch()` unconditionally sends to every handler — that specific cause was fixed, but the fix was validated only by calling the handler directly (the same blind spot Issue 41 names), so it missed a second, independent bug: `_dispatch()`'s own first parameter is named `verb`, and `k_auth_can_i`'s wrapper also passes `verb=<the RBAC verb>` as a kwarg — a direct name collision that crashes every call regardless of the `discovery_cache` fix. The tool has never worked through a real MCP client at any point in this FR's history. Issue 41 proposes the structural test (calling through `_dispatch()`, not the handler directly) that would have caught both bugs.
