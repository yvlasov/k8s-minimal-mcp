...
| `k_get_helm_release` | `release`, `namespace` | `revision`, `include_manifest` | N/A — bounded decoded summary by default; full rendered manifest only on explicit `include_manifest=True`. Decoded `values` still returned unredacted, but the exposure is now closed via access level rather than content filtering — see `KNOWN_ISSUES.md` Issue 39 (resolved: gated to `admin`, Option B). Second named R1 exception (§15 FR12) — resource-specific by design, Helm-release-`Secret`-only. | admin |
| `k_auth_can_i` | none | `verb`, `resource`, `name`, `namespace`, `as_user`, `as_group`, `list_all` | N/A — single-check mode returns `{"allowed": bool}` plus the literal command; `list_all=True` returns `{"permissions": {...}}`. Two dispatch-path bugs (`discovery_cache` kwarg mismatch, then a `_dispatch()`/`verb` parameter collision) fixed across three commits — see `KNOWN_ISSUES.md` Issue 40 for the full history — and independently confirmed working end-to-end via a live server call, both modes. | readonly |
...
**Status against current `main` (2026-09-25), post-Issue 39/41:**
- **Tool count:** met — 6 tools at `readonly`, 9 at `readwrite`, 12 at `admin` (limit 12), confirmed by directly building the real server for all three access levels. `k_get_helm_release` moved from `readonly` to `admin`-only (Issue 39), which is why the `readonly` count dropped from 7 to 6 and now has headroom again; `admin` is unchanged, still exactly 12/12. All 12 confirmed working end-to-end, including `k_auth_can_i` (Issue 40) and the access-gate change itself (Issue 39), both independently live-verified.
...
**Related finding, surfaced while designing this FR — now resolved (see Issue 35 in `KNOWN_ISSUES.md`).** `output/pruning.py`'s `prune()` had **no special handling for `Secret` resources at all** — confirmed by reading the file. This meant `k_get`, `k_apply`, `k_patch`, and `k_delete` returned a Secret's full base64-encoded `.data` map verbatim in their responses, for *any* Secret fetch or mutation, `src_file` or not. This directly undercut the motivation behind both this FR and FR9: there's no point routing a Secret's content around the model at the `apply` step if the response from that same `apply` call echoes the resulting object's `.data` field straight back. Resolved by Issue 35's hard-block Secret redaction in `prune()` — when `kind == "Secret"`, `.data` and `.stringData` are replaced with `{"redacted_keys": [...]}` (key names only, never values).
...
- **Resolved via `KNOWN_ISSUES.md` Issue 39 (Option B — raise the access gate):** whether the decoded `values` block needs its own redaction was originally shipped without an answer, then fixed by moving `k_get_helm_release` from `readonly` to `admin`-only in `access.py`, rather than adding content-level redaction (Option A). `values` is still returned verbatim in full — the tool's `readonly`-level exposure is what closed, not the plaintext-in-response question itself. Same posture as `k_get_secret_to_file`: risk gated by access level, not by filtering content.
...
**Shipped, and now confirmed working end-to-end — see `KNOWN_ISSUES.md` Issue 40 (2026-09-25):** the implementation itself (the `run_kubectl`-not-`_checked` exit-code handling this section anticipated as the hard part) was correct and well-tested from the start. Getting the tool actually callable took two more fixes beyond that: `handle_auth_can_i()`'s signature didn't accept the `discovery_cache` parameter `server.py`'s `_dispatch()` unconditionally sends every handler (fixed), and `_dispatch()`'s own first parameter was named `verb`, colliding with `k_auth_can_i`'s own `verb` domain argument (fixed). Both prior "fix" commits validated themselves only by calling the handler or `_dispatch()` directly rather than through a real server call, and both asserted "Verified" claims that turned out incomplete or unconfirmed — the tool was independently confirmed working, in both single-check and `--list` modes, only by building the real server and calling the registered tool through FastMCP's own dispatch. Issue 41 (still open) proposes the structural test that would have caught both bugs the first time, and prevents the next tool from repeating this exact history.
