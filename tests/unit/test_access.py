...
class TestAccessLevel:
    def test_readonly_verbs(self):
        ...
        assert verbs == frozenset({"get", "logs", "describe", "list_resources", "auth_can_i"})

    def test_readwrite_verbs(self):
        verbs = allowed_verbs(AccessLevel.READWRITE)
        assert verbs == frozenset({"get", "logs", "describe", "list_resources", "auth_can_i", "apply", "patch", "delete"})

    def test_admin_verbs(self):
        verbs = allowed_verbs(AccessLevel.ADMIN)
        assert verbs == frozenset({"get", "logs", "describe", "list_resources", "auth_can_i", "apply", "patch", "delete", "exec", "get_secret_to_file", "get_helm_release"})
    ...
    def test_get_helm_release_admin_only(self):
        """Issue 39: get_helm_release returns chart values unredacted — gated to admin."""
        assert "get_helm_release" not in allowed_verbs(AccessLevel.READONLY)
        assert "get_helm_release" not in allowed_verbs(AccessLevel.READWRITE)
        assert "get_helm_release" in allowed_verbs(AccessLevel.ADMIN)
