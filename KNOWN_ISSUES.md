...
### 40. `k_auth_can_i` is still completely broken on every real call — the `discovery_cache` fix (commit `3115af3`) addressed one cause, but a second, independent bug in `_dispatch()` itself was never found because the fix was validated only by calling the handler directly, never through the real server

**File:** `src/k8s_mcp/server.py:47,137-139` (root cause of the still-open failure), `src/k8s_mcp/tools/auth_can_i.py:84` (the earlier, now-moot fix)
**Severity:** Critical, unchanged from the original report — the tool has never worked through a real MCP client, at any point in this FR's history.
**History:** originally filed as "`handle_auth_can_i()` doesn't accept the `discovery_cache` kwarg `_dispatch()` always passes" (`TypeError: handle_auth_can_i() got an unexpected keyword argument 'discovery_cache'`). Commit `3115af3` added `*, discovery_cache: object | None = None` to `handle_auth_can_i()`'s signature and a new test (`test_discovery_cache_accepted`) that calls `handle_auth_can_i(..., discovery_cache=None)` **directly** — the exact same test pattern the original report identified as the reason the bug shipped undetected in the first place. That test passed, and the commit's own KNOWN_ISSUES.md update claimed (falsely, as it turned out) "Verified... confirmed... 304 tests pass... Committed as `<commit-hash>`" — a claim written without ever calling the tool through `_dispatch()`, and left with a literal unfilled `<commit-hash>` placeholder besides.
**Independently re-verified (2026-09-25) by actually building the real FastMCP server via `server.main()` and invoking the registered `k_auth_can_i` tool the way a real MCP client would — confirmed it still crashes, with a DIFFERENT error:**
```
TypeError: _dispatch() got multiple values for argument 'verb'
```
**Root cause of this second bug:** `_dispatch(verb: str, handler, context, discovery_cache, allow_namespaces, **kwargs)` (`server.py:47`) uses `verb` as its OWN first positional parameter name — meaning "which dispatch verb this tool is" (e.g. `"auth_can_i"`). The `auth_can_i` wrapper (`server.py:137-139`) calls `_dispatch("auth_can_i", handle_auth_can_i, context, discovery_cache, allow_namespaces, verb=verb, resource=resource, ...)` — passing the string `"auth_can_i"` positionally into `_dispatch`'s `verb` parameter, and *also* passing `verb=verb` as a keyword (the RBAC verb the caller asked about, e.g. `"get"`) — a direct name collision on the same parameter. `k_auth_can_i` is the only tool whose domain parameters happen to include one literally named `verb`, which is why no other tool's wrapper hits this. Completely independent of the `discovery_cache` bug; commit `3115af3` never touched `server.py` and could not have fixed this.
**Why the "fix" that shipped couldn't have caught this:** this is precisely the scenario Issue 41 (below) was written to prevent — a regression test that calls the handler directly proves the handler's own signature is fine, but says nothing about whether the *wrapper that calls `_dispatch()`* is correct. The `discovery_cache` fix's own regression test repeated the exact evasion pattern the bug report it was fixing had just diagnosed.
...
**Confirmed still needed (2026-09-25):** the attempted fix for Issue 40 added a new test that again called the handler directly instead of through `_dispatch()`, and that fix shipped a "Verified" claim without ever discovering `k_auth_can_i` still crashes — this time on a second, unrelated bug (`_dispatch()`'s own `verb` parameter colliding with the RBAC `verb` domain argument). This is exactly the failure this issue predicted, recurring even in the act of trying to fix the first instance of it.
